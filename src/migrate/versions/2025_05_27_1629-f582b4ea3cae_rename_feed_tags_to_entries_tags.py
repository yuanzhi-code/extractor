...
def upgrade() -> None:
    ...
    with op.batch_alter_table('entries_tags', schema=None) as batch_op:
        batch_op.create_index('idx_entries_tags_entry_id', ['entry_id'], unique=False)
        batch_op.create_index('idx_entries_tags_tag_id', ['tag_id'], unique=False)

    op.drop_table('content_tags')
    with op.batch_alter_table('tags', schema=None) as batch_op:
        batch_op.add_column(sa.Column('parent_id', sa.Integer(), nullable=True))
        batch_op.add_column(sa.Column('level', sa.SmallInteger(), nullable=False))
        batch_op.create_index('idx_tags_level', ['level'], unique=False)
        batch_op.create_index('idx_tags_parent_id', ['parent_id'], unique=False)
        batch_op.create_unique_constraint('uq_tags_name_parent', ['name', 'parent_id'])
