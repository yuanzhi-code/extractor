...
def upgrade() -> None:
    ...
    with op.batch_alter_table('entries_categories', schema=None) as batch_op:
        ...
        batch_op.create_index('idx_entries_categories_category', ['category'], unique=False)
        batch_op.drop_column('category_id')

    with op.batch_alter_table('entry_scores', schema=None) as batch_op:
        batch_op.add_column(sa.Column('score', sa.String(length=25), nullable=False))
...
def downgrade() -> None:
    ...
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('entry_scores', schema=None) as batch_op:
        batch_op.add_column(sa.Column('quality_score', sa.INTEGER(), nullable=False))
        batch_op.add_column(sa.Column('relevance_score', sa.INTEGER(), nullable=False))
        batch_op.add_column(sa.Column('final_score', sa.INTEGER(), nullable=False))
        batch_op.drop_index('idx_entry_scores_score')
        batch_op.create_index(batch_op.f('idx_entry_scores_final_score'), ['final_score'], unique=False)
    ...
    with op.batch_alter_table('entries_categories', schema=None) as batch_op:
        ...
        batch_op.create_index(batch_op.f('idx_entries_categories_category_id'), ['category_id'], unique=False)
