...
class WebContentExtractor:
    ...
    async def __aenter__(self):
        ...
        self.crawler = AsyncWebCrawler(
            ...
            block_resources=["image", "media", "font", "stylesheet"]  # 阻止资源加载
        )
    ...
    async def extract_main_content(self, url: str, use_readability: bool = True) -> Dict[str, Any]:
        """提取网页主要内容"""
        try:
            # 爬取配置
            crawl_config = {
            ...
            if not markdown_content and result.cleaned_html:
                markdown_content = self._html_to_markdown_simple(result.cleaned_html)
            ...
            return {
                ...
                "extracted_at": result.extracted_at if hasattr(result, 'extracted_at') else None
            }
    ...
    async def extract_multiple_urls(self, urls: list, max_concurrent: int = 3) -> Dict[str, Any]:
        """批量提取多个URL的内容"""
...
# 使用示例函数
async def scrape_website_to_markdown(url: str, use_readability: bool = True) -> Dict[str, Any]:
    """爬取单个网站并转换为Markdown"""
    async with WebContentExtractor() as extractor:
        result = await extractor.extract_main_content(url, use_readability)
        return result

async def scrape_multiple_websites(urls: list, max_concurrent: int = 3) -> Dict[str, Any]:
    """批量爬取多个网站"""
    async with WebContentExtractor() as extractor:
...
async def main():
    ...
    # 单个URL爬取
    print("=== 单个URL爬取示例 ===")
    single_result = await scrape_website_to_markdown("http://www.geekpark.net/news/349159")
    ...
    print("\n=== 批量URL爬取示例 ===")
    # 批量URL爬取
    batch_results = await scrape_multiple_websites(test_urls[:2])  # 限制测试数量
    ...
    for url, result in batch_results.items():
        ...
        if result["success"]:
            ...
            print(f"  内容预览: {result['content'] if result['content'] else 'No content'}...")
        else:
    ...
    print(f"快速爬取结果: {quick_content if quick_content else 'No content'}...")
